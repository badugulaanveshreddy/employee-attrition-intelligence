...
import os
st.set_page_config(page_title = "EMPLOYEE ATTRITION INTELLIGENCE",page_icon="📊",
...
DATA_PATH = os.path.join(BASE_DIR, "data", "processed", "hr_featured.csv")
...
st.markdown("""AI-powered Employee Attrition Prediction Dashboard""")
...
with col2:
    st.metric(
        ...
        value=f"{df['Attrition'].mean()*100:.1f}%"
    )

with col3:
    st.metric(
        label="🎂 Average Age",
        value=f"{df['Age'].mean():.0f} Years"
    )
...
st.caption("Explore workforce composition and employee attrition patterns.")
...
# ------------------------------------------------------------
# CHART 2 — ATTRITION DISTRIBUTION
# ------------------------------------------------------------
...
with col2:
    ...
    # Convert values to readable labels
    attrition_counts["Status"] = attrition_counts["Attrition"].map({
        0: "Stayed",
        1: "Left",
        "0": "Stayed",
        "1": "Left",
...
st.caption("Identify departments and roles with higher employee attrition.")
...
with st.sidebar:
     ...
     with st.expander("👤 Personal Details", expanded=True):
        ...
        education_field = st.selectbox(
            "Education Field",list(encoders["EducationField"].classes_)
            )
     ...
     with st.expander("💰 Salary Details"):
          monthly_income = st.number_input("Monthly Income",min_value = 1000,max_value = 5000)
...
st.caption(
    "Top factors contributing to the employee attrition prediction."
)
